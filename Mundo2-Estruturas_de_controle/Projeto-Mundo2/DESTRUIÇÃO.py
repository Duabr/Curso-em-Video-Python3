# Jogo de combate em turnos, com diferentes habilidades. Aproveite!
...
while j_confirmacao_atributos!='1':
    ...
    while (j_atributos!=0) or (0>j_agi or 0>j_des or 0>j_lut or 0>j_sab or 0>j_vit) or (j_agi>10 or j_des>10 or j_lut>10 or j_sab>10 or j_vit>10):
        ...
        if j_agi==-2:
            cheats_ativados=1
            print(f'{c["neg"]}TRAPAÇAS ATIVADAS{c["n"]}')
            sleep(1)
            escolha_cheats=input('''[1] Atributos do inimigo personalizados
[2] Escolha das ações do inimigo
[3] Ignorar cooldown das habilidades do jogador
[4] Ignorar cooldown das habilidades do inimigo
[5] Mostrar PV do inimigo

Digite os números correspondentes às trapaças que deseja ativar: ''')
            print(f'{c["neg"]}FEITO{c["n"]}')
            sleep(1)
            j_agi=int(input('AGILIDADE: '))
        if j_agi==-1:
...
i_cheat_perso_atri=i_cheat_escolher_acoes=i_cheat_ignorar_cooldown=j_cheat_ignorar_cooldown=i_cheat_mostrar_pv=0
if '1' in escolha_cheats:
    i_cheat_perso_atri=1
if '2' in escolha_cheats:
    i_cheat_escolher_acoes=1
if '3' in escolha_cheats:
    j_cheat_ignorar_cooldown=1
if '4' in escolha_cheats:
    i_cheat_ignorar_cooldown=1
if '5' in escolha_cheats:
    i_cheat_mostrar_pv=1
...
j_hab5_aumento_resistencia = 6 + j_sab // 9 * 4
...
j_hab6_dano = (40 + j_sab // 10 * 25) + (j_des * (8 + (j_sab // 10 * 5)))
j_hab6_diminuicao_resistencia = 6 + j_sab // 10 * 3
...
j_hab5_cooldown_atual=j_hab4_cooldown_atual=j_hab3_cooldown_atual=j_hab2_cooldown_atual=j_hab6_cooldown=0

i_confirmacao_atributos=''
i_atributos=1
while i_cheat_perso_atri==1 and i_confirmacao_atributos!='1':
    print(f'{c["neg"]}Agora distribua os pontos dos atributos do inimigo:{c["n"]}')
    while True:
        i_atributos=15
...
i_hab2_duracao_base = 2 + i_sab // 6 * 1
i_hab2_duracao_atual = (2 + i_sab // 6 * 1) + 1
...
sleep(2)
print(f'{c["n"]}')
j_resistencia=i_resistência=turno=0
j_atordoado=2
...
while True:
    ...
    if j_hab5_duracao_atual<j_hab5_duracao_base:
        ...
        if i_hab3_duracao_atual<i_hab3_duracao_base:
            ...
            sleep(2)
    if j_hab6_duracao_atual<j_hab6_duracao_base:
    ...
    j_hab_disponiveis='0 1 '
    if (j_hab2_cooldown_atual==0) or (j_cheat_ignorar_cooldown==1):
        j_hab_disponiveis+='2 '
    if (j_hab3_cooldown_atual==0 or j_cheat_ignorar_cooldown==1) and j_sab>=1:
        j_hab_disponiveis+='3 '
    if (j_hab4_cooldown_atual==0 or j_cheat_ignorar_cooldown==1) and j_sab>=2:
        j_hab_disponiveis+='4 '
    if (j_hab5_cooldown_atual==0 or j_cheat_ignorar_cooldown==1) and j_sab>=3:
        j_hab_disponiveis+='5 '
    if (j_hab6_cooldown==0 or j_cheat_ignorar_cooldown==1) and j_sab>=4:
        j_hab_disponiveis+='6 '
    ...
    if j_num_habs_melhorias >= 11 and j_hab5_cooldown_atual == 0:
        ...
        j_acoes += '[5] Descanso\n'
    if j_num_habs_melhorias >= 12 and j_hab6_cooldown == 0:
        j_acoes += f'[6] {c["rox"]}Destruição{c["n"]}\n'
    elif j_num_habs_melhorias >= 12 and j_hab6_cooldown == 1:
        j_acoes += f'[6] {c["neg/sub"]}Destruição{c["n"]}\n'
    elif j_num_habs_melhorias >= 6 and j_hab6_cooldown == 1:
    ...
    if j_atordoado==0 and j_morrendo=="vivo":
        print(f'{c["vm"]}POR ENQUANTO, VOCÊ NÃO CONSEGUE FAZER NADA{c["n"]}')
        sleep(2)
    elif j_atordoado==1 and j_morrendo=="vivo":
        print(f'{c["vd"]}VOCÊ CONSEGUE AGIR AGORA!{c["n"]}')
        sleep(2)

    j_escolha = -1
    print('')
    print(f'{c["vd"]}PV atual: {j_vida_atual}/{j_vida_max}{c["n"]}')
    if j_atordoado>=1 or j_morrendo=="morto":
        print(j_acoes)
        while True:
            j_escolha = input('''Escolha a habilidade que quer usar: ''').strip()
            if j_escolha not in j_hab_disponiveis or j_escolha=='':
        ...
        if int(j_escolha) == 1:
            if randint(1,j_chance_acerto_atual)>=randint(1,i_chance_esquiva_atual) or j_hab6_duracao_atual<j_hab6_duracao_base:
            ...
            if j_hab2_chance_esquiva_extra > 0:
                ...
                sleep(2)
            j_hab2_cooldown_atual = j_hab2_cooldown_base-j_hab2_cooldown_base*j_cheat_ignorar_cooldown
            j_hab2_duracao_atual = 0
            
        elif int(j_escolha) == 3:
            j_hab3_cooldown_atual = j_hab3_cooldown_base-j_hab3_cooldown_base*j_cheat_ignorar_cooldown
            j_hab3_num_acertos = 0
            for _ in range(j_hab3_num_golpes):
                if randint(1,j_chance_acerto_atual-j_hab3_chance_acerto_perdida)>=randint(1,i_chance_esquiva_atual) or j_hab6_duracao_atual<j_hab6_duracao_base:
            ...
            sleep(2)
            j_hab4_cooldown_atual = j_hab4_cooldown_base-j_hab4_cooldown_base*j_cheat_ignorar_cooldown
            ...
            print(f'{c["ama"]}Durante {c["sub"]+str(j_hab5_duracao_base)+c["ama"]} turnos, você receberá {c["sub"]+str(j_hab5_aumento_resistencia)+c["ama"]} pontos de dano a menos e sua vida será curada em {c["sub"]+str(j_hab5_cura)+c["ama"]} pontos por turno.{c["n"]}')
            sleep(3)
            j_hab5_cooldown_atual = j_hab5_cooldown_base-j_hab5_cooldown_base*j_cheat_ignorar_cooldown
            j_hab5_duracao_atual = 0
            j_resistencia += j_hab5_aumento_resistencia

        elif int(j_escolha) == 6:
            j_hab6_cooldown = 1-1*j_cheat_ignorar_cooldown
            if i_hab5_imune_a_danos > 0:
    ...
    if i_vida_atual==i_vida_max or i_hab2_duracao_atual==i_hab2_duracao_base:
    ...
    if i_vida_atual<=(150+30*i_vit)/2:
        print('Ele parece bem machucado...')
        sleep(2)
    print('')
    if i_cheat_mostrar_pv==1:
        print(f'{c["vd"]}PV atual: {i_vida_atual}/{150+30*i_vit}{c["n"]}')

    if i_atordoado<j_hab6_duracao_base and i_morrendo=="vivo":
        print(f'{c["rox"]}POR ENQUANTO, O INIMIGO NÃO CONSEGUE AGIR!{c["n"]}')
        sleep(2)
    elif i_atordoado==j_hab6_duracao_base and i_morrendo=="vivo":
        print(f'{c["vm"]}O INIMIGO CONSEGUE AGIR AGORA!{c["n"]}')
        sleep(2)
    
    i_hab_disponiveis = '1 '
    if (i_hab2_cooldown_atual==0 or i_cheat_ignorar_cooldown==1):
        i_hab_disponiveis += '2 '
    if (i_hab3_cooldown_atual==0 or i_cheat_ignorar_cooldown==1) and (i_sab>=1):
        i_hab_disponiveis += '3 '
    if (i_hab4_cooldown_atual==0 or i_cheat_ignorar_cooldown==1) and (i_sab>=2):
        i_hab_disponiveis += '4 '
    if ((i_hab5_cooldown==1 and i_vida_atual<30) or (i_cheat_ignorar_cooldown==1)) and (i_sab>=3):
        i_hab_disponiveis += '5 '
    if ((i_hab6_cooldown_atual==0 and j_vida_atual>i_hab6_margem_max_dano/2) or (i_cheat_ignorar_cooldown==1)) and (i_sab>=4):
        i_hab_disponiveis += '6 '

    i_escolha=i_cheat_acoes=''
    if i_cheat_escolher_acoes==1:
        if i_num_habs_melhorias>=7:
            i_cheat_acoes+=f'[1] {c["vm"]}Garra{c["n"]}\n'
        else:
            i_cheat_acoes+=f'[1] Garra\n'
        if i_num_habs_melhorias>=8 and '2' in i_hab_disponiveis:
            i_cheat_acoes+=f'[2] {c["az"]}Espinhos{c["n"]}\n'
        elif i_num_habs_melhorias>=8 and '2' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[2] {c["neg/sub"]}Espinhos{c["n"]} ({i_hab2_cooldown_atual})\n'
        elif '2' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[2] {c["neg"]}Espinhos{c["n"]} ({i_hab2_cooldown_atual})\n'
        else:
            i_cheat_acoes+='[2] Espinhos\n'
        if i_num_habs_melhorias>=9 and '3' in i_hab_disponiveis:
            i_cheat_acoes+=f'[3] {c["ama"]}Ácido{c["n"]}\n'
        elif i_num_habs_melhorias>=9 and '3' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[3] {c["neg/sub"]}Ácido{c["n"]} ({i_hab3_cooldown_atual})\n'
        elif i_num_habs_melhorias>=3 and '3' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[3] {c["neg"]}Ácido{c["n"]} ({i_hab3_cooldown_atual})\n'
        elif i_num_habs_melhorias>=3:
            i_cheat_acoes+='[3] Ácido\n'
        if i_num_habs_melhorias>=10 and '4' in i_hab_disponiveis:
            i_cheat_acoes+=f'[4] {c["rox"]}Confusão{c["n"]}\n'
        elif i_num_habs_melhorias>=10 and '4' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[4] {c["neg/sub"]}Confusão{c["n"]} ({i_hab4_cooldown_atual})\n'
        elif i_num_habs_melhorias>=4 and '4' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[4] {c["neg"]}Confusão{c["n"]} ({i_hab4_cooldown_atual})\n'
        elif i_num_habs_melhorias>=4:
            i_cheat_acoes+='[4] Confusão\n'
        if i_num_habs_melhorias>=11 and '5' in i_hab_disponiveis:
            i_cheat_acoes+=f'[5] {c["vd"]}Reconstrução{c["n"]}\n'
        elif i_num_habs_melhorias>=11 and '5' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[5] {c["neg/sub"]}Reconstrução{c["n"]}\n'
        elif i_num_habs_melhorias>=5 and '5' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[5] {c["neg"]}Reconstrução{c["n"]}\n'
        elif i_num_habs_melhorias>=5:
            i_cheat_acoes+='[5] Reconstrução\n'
        if i_num_habs_melhorias>=12 and '6' in i_hab_disponiveis:
            i_cheat_acoes+=f'[6] {c["pr"]}Caos{c["n"]}\n'
        elif i_num_habs_melhorias>=12 and '6' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[6] {c["neg/sub"]}Caos{c["n"]} ({i_hab6_cooldown_atual})\n'
        elif i_num_habs_melhorias>=6 and '6' not in i_hab_disponiveis:
            i_cheat_acoes+=f'[6] {c["neg"]}Caos{c["n"]} ({i_hab6_cooldown_atual})\n'
        elif i_num_habs_melhorias>=6:
            i_cheat_acoes+='[6] Caos\n'
        if i_atordoado>=j_hab6_duracao_base or i_morrendo=="morto":
            print(i_cheat_acoes)
    while (i_atordoado>=j_hab6_duracao_base or i_morrendo=="morto") and (i_escolha not in i_hab_disponiveis or i_escolha==''):
        if i_cheat_escolher_acoes==1:
            i_escolha = input('Escolha a habilidade que o inimigo usará: ').strip()
        else:
            i_escolha = choice(i_hab_disponiveis.split())

        if i_escolha not in i_hab_disponiveis or i_escolha=='':
            print(f'{c["vm"]}Escolha uma opção válida!{c["n"]}')
        else:
            break


    if (i_cheat_escolher_acoes==0 and '5' in i_hab_disponiveis and i_morrendo=="vivo") or (i_cheat_escolher_acoes==1 and i_escolha=='5'):
        print(f'{c["neg"]}O inimigo usa a habilidade "{c["vd"]+"Reconstrução"+c["neg"]}"!{c["n"]}')
        sleep(1)
        print(f'{c["vd"]}Para o seu espanto, uma parte do corpo dele é RECONSTRUÍDA. Ele acabou de se recuperar de parte do estrago sofrido?{c["n"]}')
        i_vida_atual+=i_hab5_cura
        sleep(2)
        if i_sab>=9:
            print(f'{c["vd"]}Sua pele também parece mais resistente do que antes...{c["n"]}')
            sleep(2)
            i_hab5_imune_a_danos=0
        i_hab5_cooldown=0

    elif i_escolha == '1':
        print(f'{c["neg"]}O inimigo usa a habilidade "{c["vm"]+"Garras"+c["neg"]}"!{c["n"]}')
        sleep(1)
        ...
        if (i_teste_acerto>=0 and j_hab2_duracao_atual>=j_hab2_duracao_base) or (j_atordoado==1) or (j_hab2_duracao_atual<j_hab2_duracao_base and i_teste_acerto>=0<=i_teste_acerto_extra):
            print(f'{c["vm"]}Ele usa suas GARRAS para te atacar, causando {c["sub"]+str(i_hab1_dano-j_resistencia)+c["vm"]} pontos de dano.{c["n"]}')
            j_vida_atual-=i_hab1_dano-j_resistencia
        else:
            print(f'{c["vd"]}Ele usa suas {c["vm"]+"GARRAS"+c["vd"]} para te atacar, mas você consegue esquivar!{c["n"]}')
        sleep(2)
    
    elif i_escolha == '2':
        print(f'{c["neg"]}O inimigo usa a habilidade "{c["az"]+"Espinhos"+c["neg"]}"!{c["n"]}')
        sleep(1)
        print(f'{c["az"]}ESPINHOS surgem na pele dele. Atacá-lo agora será perigoso!{c["n"]}')
        sleep(2)
        ...
        print(f'{c["neg"]}O inimigo usa a habilidade "{c["ama"]+"Ácido"+c["neg"]}"!{c["n"]}')
        sleep(1)
        ...
        print(f'{c["neg"]}O inimigo usa a habilidade "{c["rox"]+"Confusão"+c["neg"]}"!{c["n"]}')
        sleep(1)
        print(f'{c["rox"]}Você fica CONFUSO e perde {c["sub"]+str(i_hab4_diminuicao_chance_acerto)+c["rox"]} pontos de chance de acerto.{c["n"]}')
        sleep(2)
        ...
        print(f'{c["neg"]}O inimigo usa a habilidade "{c["pr"]+"Caos"+c["neg"]}"!{c["n"]}')
        sleep(1)
        i_teste_acerto = randint(1,i_chance_acerto_atual)-randint(1,j_chance_esquiva_atual)# Se for menor que zero, o jogador esquiva, senão o inimigo acerta.
        i_teste_acerto_extra = randint(1,i_chance_acerto_atual)-randint(1,j_chance_esquiva_atual)# Caso acerte, mais uma chance para o jogador esquivar (efeito da habilidade 2 do jogador)
        if (i_teste_acerto>=0 and j_hab2_duracao_atual>=j_hab2_duracao_base) or (j_atordoado==1) or ((j_hab2_duracao_atual<j_hab2_duracao_base) and (i_teste_acerto>=0<=i_teste_acerto_extra)):
    ...
    if j_hab2_duracao_atual<j_hab2_duracao_base and j_atordoado==0:
        ...
        if j_hab2_duracao_base-j_hab2_duracao_atual==0:
            ...
            sleep(2)
    elif (j_hab2_duracao_atual<j_hab2_duracao_base) and ('2'!=i_escolha!='4' and '5' not in i_hab_disponiveis):
    ...
    if i_morrendo=="morto" and j_morrendo=="vivo":
        ...
        sleep(2)
        break
    elif j_morrendo=='morto'==i_morrendo:
